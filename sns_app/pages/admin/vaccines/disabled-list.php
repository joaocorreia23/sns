...
<?php $page_name = "Listagem de Vacinas - Desativadas" ?>
		...
		var datatableServerSide = (function() {
			...
			var initDatatable = () => {
				dt = $("#datatable").DataTable({
					...
					stateSave: false,
					ajax: {
						url: "http://localhost:3000/api/vaccines/table/disabled",
						type: "GET",
					},
					columns: [{
							data: "vaccine_name"
						...
						},
						{
							...
							render: (data, type, row) => {
								...
								return `
									<div class="d-inline-flex align-items-center">                                
										<div class="d-flex justify-content-center flex-column">
											<span class="badge badge-danger px-2 py-2">Não Permite Vacinar</span>
										</div>
									</div>
								`;
							...
							render: (data, type, row) => {
								return `
									<div>
										...
										<button type="button" data-id="${row.hashed_id}" data-name="${row.vaccine_name}" data-datatable-action="delete-row" class="btn btn-icon btn-bg-light btn-color-success btn-active-light-success rounded w-35px h-35px"><i class="ki-outline ki-check fs-2"></i></button>
									</div>
								`;
							},
						},
			...
			var handleDeleteRows = () => {
				...
				$("#datatable").on("click", "[data-datatable-action='delete-row']", (e) => {
					...
					Swal.fire({
						icon: "warning",
						title: "Ativar Vacina",
						text: "Tem a certeza que deseja ativar a Vacina (" + name + ") ?",
						...
						if (result.isConfirmed) {
							...
							fetch("http://localhost:3000/api/vaccines/activate", options)
								.then((response) => {
