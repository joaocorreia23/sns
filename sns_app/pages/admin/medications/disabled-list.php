...
<?php $page_name = "Listagem de Medicação - Desativadas" ?>
											...
											<table id="datatable" class="table align-middle gs-0 gy-4">
												<thead>
													<tr class="fw-bold text-muted bg-light">
														...
														<th class="ps-4 fs-6 min-w-100px rounded-start" data-priority="4">Estado</th>
		...
		var datatableServerSide = (function() {
			...
			var initDatatable = () => {
				dt = $("#datatable").DataTable({
					...
					stateSave: false,
					ajax: {
						url: "http://localhost:3000/api/medications/table/disabled",
						type: "GET",
					},
					columns: [{
						...
						},
						{
							...
							render: (data, type, row) => {
								...
								return `
									<div class="d-inline-flex align-items-center">                                
										<div class="d-flex justify-content-center flex-column">
											<span class="badge badge-danger px-2 py-2">Não Permite Medicar</span>
										</div>
									</div>
								`;
							...
							render: (data, type, row) => {
								return `
									<div>
										...
										<button type="button" data-id="${row.hashed_id}" data-name="${row.medication_name}" data-datatable-action="delete-row" class="btn btn-icon btn-bg-light btn-color-success btn-active-light-success rounded w-35px h-35px"><i class="ki-outline ki-check fs-2"></i></button>
									</div>
								`;
							},
						},
			...
			var handleDeleteRows = () => {
				...
				$("#datatable").on("click", "[data-datatable-action='delete-row']", (e) => {
					...
					Swal.fire({
						...
						text: "Tem a certeza que deseja ativar o Medicamento (" + name + ") ?",
						...
						if (result.isConfirmed) {
							...
							fetch("http://localhost:3000/api/medications/activate", options)
								.then((response) => {
