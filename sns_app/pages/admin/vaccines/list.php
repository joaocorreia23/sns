...
<?php $page_name = "Listagem de Vacinas" ?>
		...
		var datatableServerSide = (function() {
			...
			var initDatatable = () => {
				dt = $("#datatable").DataTable({
					...
					stateSave: false,
					ajax: {
						url: "http://localhost:3000/api/vaccines/table",
						type: "GET",
					},
					columns: [{
							data: "vaccine_name"
						...
						},
						{
							...
							render: (data, type, row) => {
								...
								return `
									<div class="d-inline-flex align-items-center">                                
										<div class="d-flex justify-content-center flex-column">
											<span class="badge badge-success px-2 py-2">Permite Vacinar</span>
										</div>
									</div>
								`;
							...
							render: (data, type, row) => {
								return `
									<div>
										...
										<button type="button" data-id="${row.hashed_id}" data-name="${row.vaccine_name}" data-datatable-action="delete-row" class="btn btn-icon btn-bg-light btn-color-danger btn-active-light-danger rounded w-35px h-35px"><i class="ki-outline ki-trash fs-2"></i></button>
									</div>
								`;
							},
						},
