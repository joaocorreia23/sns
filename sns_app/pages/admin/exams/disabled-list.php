...
<?php $page_name = "Listagem de Prescrição de Exames - Desativadas" ?>
											...
											<table id="datatable" class="table align-middle gs-0 gy-4">
												<thead>
													<tr class="fw-bold text-muted bg-light">
														...
														<th class="ps-4 fs-6 min-w-200px rounded-start" data-priority="2">Estado</th>
														<th class="ps-4 fs-6 min-w-100px rounded-start" data-priority="3">Data Criação</th>
														<th class="pe-4 fs-6 min-w-200px text-sm-end rounded-end" data-priority="4">Ações</th>
													</tr>
												</thead>
												<tbody></tbody>
											</table>
		...
		var datatableServerSide = (function() {
			...
			var initDatatable = () => {
				dt = $("#datatable").DataTable({
					...
					stateSave: false,
					ajax: {
						url: "http://localhost:3000/api/exams/table/disabled",
						type: "GET",
					},
					columns: [{
							data: "exam_name"
					...
					columnDefs: [{
						...
						},
						{
                            ...
                            <div class="d-inline-flex align-items-center">                                
                                <div class="d-flex justify-content-center flex-column">
                                    <span class="badge badge-danger px-2 py-2">Impossível Prescrever</span>
                                </div>
                            </div>
                        `
								}
						...
						},
						{
							...
							<a href="edit?id=${row.hashed_id}" class="btn btn-icon btn-bg-light btn-color-primary btn-active-light-primary rounded w-35px h-35px me-1"><i class="ki-outline ki-notepad-edit fs-2"></i></a>
                                <button type="button" data-id="${row.hashed_id}" data-name="${row.exam_name}" data-datatable-action="delete-row" class="btn btn-icon btn-bg-light btn-color-success btn-active-light-success rounded w-35px h-35px"><i class="ki-outline ki-check fs-2"></i></button>
                            </div>
                        `
							},
						},
			...
			var handleDeleteRows = () => {
				...
				$("#datatable").on("click", "[data-datatable-action='delete-row']", (e) => {
					...
					Swal.fire({
						icon: "warning",
						title: "Ativar Prescrição - Exame",
						text: "Tem a certeza que deseja ativar a Prescrição - Exame (" + name + ") ?",
						...
						if (result.isConfirmed) {
							...
							fetch("http://localhost:3000/api/exams/activate", options)
								.then((response) => {
