...
<?php $page_name = "Listagem de Utilizadores - Desativados" ?>
		...
		var datatableServerSide = (function() {
			...
			var initDatatable = () => {
				dt = $("#datatable").DataTable({
					...
					stateSave: false,
					ajax: {
						url: "http://localhost:3000/api/users/table/disabled",
						type: "GET",
					},
					columns: [{
							data: "first_name"
							...
							render: (data, type, row) => {
								return `
									<div>
										...
										<button type="button" data-id="${row.hashed_id}" data-name="${row.email}" data-datatable-action="delete-row" class="btn btn-icon btn-bg-light btn-color-success btn-active-light-success rounded w-35px h-35px"><i class="ki-outline ki-check fs-2"></i></button>
									</div>
								`;
							},
						},
			...
			var handleDeleteRows = () => {
				...
				$("#datatable").on("click", "[data-datatable-action='delete-row']", (e) => {
					...
					Swal.fire({
						...
						text: "Tem a certeza que deseja ativar o Utilizador (" + name + ") ?",
						...
						if (result.isConfirmed) {
							...
							fetch("http://localhost:3000/api/users/activate", options)
								.then((response) => {
